...
void conting(cListaDupEnc* Lista, int tamanhoEntrada, int palavraInicio, int palavraFim) {
  ...
  for (int i = 0; i < tamanhoEntrada;i++) {
    if(baldes[aux1->getDataDeAdicao()[palavraFim - 1] - '0'] == NULL) {
      baldes[aux1->getDataDeAdicao()[palavraFim - 1] - '0'] = aux1;
      cNo* aux2 = aux1->getProx();
      aux1->setAnte(NULL);
      aux1->setProx(NULL);
      aux1 = aux2;
    }
    else if(baldes[aux1->getDataDeAdicao()[palavraFim - 1] - '0'] != NULL) {
      cNo* aux3 = baldes[aux1->getDataDeAdicao()[palavraFim - 1] - '0'];
      cNo* aux2 = aux1->getProx();
      aux1->setProx(NULL);
      aux1->setAnte(NULL);
